...
We will see a quick setup to configure the ``tool_gpg`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_gpg`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_gpg`` formulas under ``/srv/salt/tool_gpg/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_gpg/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_gpg/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_gpg/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_gpg/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_gpg`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_gpg`` formulas under ``/srv/salt/tool_gpg/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_gpg/parameters/roles
...
We create a configuration for the role ``tool_gpg/server`` in ``/srv/salt/tool_gpg/parameters/roles/tool_gpg/server.yaml``:
...
We create another configuration for the role ``tool_gpg/client`` in ``/srv/salt/tool_gpg/parameters/roles/tool_gpg/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_gpg/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_gpg/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_gpg/parameters/dns:domain/  '/srv/salt/tool_gpg/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-gpg-as-json:
      test.nop:
        - name: {{ gpg | json }}
...
.. code-block:: sls
    ...
    gpg-config-file-file-managed:
      file.managed:
        - name: {{ gpg.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='gpg-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``gpg`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ gpg['winner'] }}
